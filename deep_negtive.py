...
def input_pipeline(filenames, batch_size, n_input, num_epochs=None, infer_mode=False):
	...
	if(infer_mode):
		...
		example_batch, mask_batch = tf.train.batch(
			example, batch_size=batch_size, capacity=capacity,num_threads = num_threads, allow_smaller_final_batch=infer_mode)
...
def loss_rsme(x, reconstruction, mask = None):
	if mask is not None:
		return 0.5 * tf.reduce_sum(tf.pow(tf.mul(tf.sub(reconstruction, x),mask), 2.0))
	return 0.5 * tf.reduce_sum(tf.pow(tf.sub(reconstruction, x), 2.0))

def loss_negtive(x, y, positve):
	if positve is True:
		return tf.reduce_sum(-tf.log(tf.sigmoid(tf.reduce_sum(tf.mul(x,y), 1))))
	else:
		return tf.reduce_sum(-tf.log(tf.sigmoid(tf.reduce_sum(-tf.mul(x,y), 1))))
...
def pretrain():
	with tf.Graph().as_default() as g:
		...
		input_, mask= input_pipeline([directory+"/data.tfrecords"], batch_size=batch_size, n_input = FLAGS.image_pixels)  
		record = input_pipeline2(
			[directory+"/negtive.tfrecords"], batch_size=batch_size, n_input = FLAGS.image_pixels) 
		input_train, _ = input_pipeline([directory+"/train.tfrecords"], batch_size=1, n_input = FLAGS.image_pixels)   
		input_test, _ = input_pipeline([directory+"/test.tfrecords"], batch_size=1, n_input = FLAGS.image_pixels)
		...
		try:
			for i in xrange(len(ae_shape) - 2):
				...
				with tf.variable_scope("pretrain_{0}".format(n)):
					...
					for step in xrange(pretraining_epochs * num_data):
			...
			sess.run(tf.initialize_variables([variable for variable in variables_after if variable not in variables_before]))
			print("\n\n")
			print("| Combine Step    | Local Loss	   |   Epoch  |")
			print("|-----------------|---------------|----------|")
			for step in xrange(combine_epochs * num_data):
			...
			print("\n\n")
			print("| Finetuning Step | negtive Loss  | reconst. Loss | total Loss    |   Epoch  |")
			print("|-----------------|---------------|---------------|---------------|----------|")
			for step in xrange(finetuning_epochs * num_finetune):
			...
			with open(directory+"/train.log", 'w') as f:       
			...
			with open(directory+"/test.log", 'w') as f:
